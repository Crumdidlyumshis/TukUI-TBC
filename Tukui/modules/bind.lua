	...
	if not bind.loaded then
		...
		GameTooltip:HookScript("OnUpdate", function(self, e)			
			...
			elseif ( self.comparing and not IsModifiedClick("COMPAREITEMS")) then				
				-- hides comapre item ToolTip even if the user requests it with SHIFT modifier
		...
		local hideFunc = function(self) 
			ShoppingTooltip1:Hide();
			ShoppingTooltip2:Hide(); 
		end
		GameTooltip:HookScript("OnHide",hideFunc)

		-- no hooksecurefunc for this shit
		--hooksecurefunc(GameTooltip, "Hide", function(self) for _, tt in pairs(self.shoppingTooltips) do tt:Hide() end end)
		--ChatFrame1:AddMessage("SetScript time")
		bind:SetScript("OnEvent", function(self) self:Deactivate(false) end)
		bind:SetScript("OnLeave", function(self) self:HideFrame() end)
		bind:SetScript("OnKeyUp", function(self, key) bind:Listener(key) end)
		bind:SetScript("OnMouseUp", function(self, key) bind:Listener(key) end)
		bind:SetScript("OnMouseWheel", function(self, delta) if delta>0 then bind:Listener("MOUSEWHEELUP") else bind:Listener("MOUSEWHEELDOWN") end end)

		function bind:Update(b, spellmacro)
			--ChatFrame1:AddMessage('SHIT UPDATE' .. b:GetName() ' - ' .. ' - ' .. spellmacro:GetName())
		...
		function bind:Listener(key)
			--ChatFrame1:AddMessage('MOTHER FUCKER - ' .. key)
		...
		-- ChatFrame1:AddMessage(tostring(button))			
		...
		local function register(val)
			if val.IsProtected and val.GetObjectType and val.GetScript and val:GetObjectType()=="CheckButton" and val:IsProtected() then
				-- ChatFrame1:AddMessage("found - " .. tostring(val:GetName()) .. ' - ' .. tostring(val:GetScript("OnClick")))
				local script = val:GetScript("OnClick")
				-- test hack der bare linker alt til button
				-- if script==button then
					-- ChatFrame1:AddMessage("Added!")
