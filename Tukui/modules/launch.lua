...
local function install()
	...
	if (TukuiCF.chat.enable == true) and (not IsAddOnLoaded("Prat") or not IsAddOnLoaded("Chatter")) then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			local chatName = GetChatWindowInfo(chatFrameId)
			
			--frame:SetSize(ChatLeft:GetWidth() - 6, ChatLeft:GetHeight() - 6)
			frame:SetWidth(ChatLeft:GetWidth() - 6)
			frame:SetHeight(ChatLeft:GetHeight() - 6)
			
			-- this is the default width and height of tukui chats.
			--SetChatWindowSavedDimensions(chatFrameId, ChatLeft:GetWidth() - 6, ChatLeft:GetHeight() - 6)
			...
			elseif i == 4 and chatName == "Loot" then
				frame:ClearAllPoints()
				frame:SetPoint("BOTTOM", ChatRight, "BOTTOM", 0, TukuiDB.Scale(4))
				frame:Hide()
			end
...
TukuiOnLogon:SetScript("OnEvent", function(self, event)
	...
	ChatFrame1:AddMessage("Welcome to Zarlie Edit v1, "..name.."!", 0, .8, 1)
end)
