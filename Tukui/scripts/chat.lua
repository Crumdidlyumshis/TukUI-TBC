...
-- relater til "goto bottom of chatframe"
...
-- TBC version to change editbox
local function SetupEditBoxTBC()
	-- Kills off the retarded new circle around the editbox
	TukuiDB.Kill(_G["ChatFrameEditBoxLeft"])	
	TukuiDB.Kill(_G["ChatFrameEditBoxRight"])

	-- Kill off editbox artwork
	local a, b, c = select(6, _G["ChatFrameEditBox"]:GetRegions()); TukuiDB.Kill (a); TukuiDB.Kill (b); TukuiDB.Kill (c)
	...
	TukuiChatchatEditBox = _G["ChatFrameEditBox"]
	...
	_G["ChatFrameEditBox"]:SetPoint("TOPLEFT", EditBoxDummy, TukuiDB.Scale(2), TukuiDB.Scale(-2))
	_G["ChatFrameEditBox"]:SetPoint("BOTTOMRIGHT", EditBoxDummy, TukuiDB.Scale(-2), TukuiDB.Scale(2))

	local function colorize(r,g,b)
	 	edit:SetBackdropBorderColor(r, g, b)
	end

	-- TBC version since there is only one editbox!	
	hooksecurefunc("ChatEdit_UpdateHeader", function()
		local type = _G["ChatFrameEditBox"]:GetAttribute("chatType")
...
local function SetChatStyle(frame)
	...
	-- Dirty hack
	-- TBC STUFF
	if(chat == "ChatFrame1") then	
		SetupEditBoxTBC();
	end	
	_G["ChatFrameEditBox"]:ClearAllPoints();
	_G["ChatFrameEditBox"]:SetPoint("TOPLEFT", TukuiInfoLeft, TukuiDB.Scale(2), TukuiDB.Scale(-2))
	_G["ChatFrameEditBox"]:SetPoint("BOTTOMRIGHT", TukuiInfoLeft, TukuiDB.Scale(-2), TukuiDB.Scale(2))		
	...
	TukuiDB.Kill(_G[format("ChatFrame%sUpButton", id)])
	-- TukuiDB.Kill(_G[format("ChatFrame%sButtonFrameDownButton", id)])
	TukuiDB.Kill(_G[format("ChatFrame%sDownButton", id)])
	-- TukuiDB.Kill(_G[format("ChatFrame%sButtonFrameBottomButton", id)])
	TukuiDB.Kill(_G[format("ChatFrame%sBottomButton", id)])
	-- TukuiDB.Kill(_G[format("ChatFrame%sButtonFrameMinimizeButton", id)])
	--TukuiDB.Kill(_G[format("ChatFrame%sMinimizeButton", id)])
...
local function SetupChatPosAndFont(self)
	...
	for i = 1, NUM_CHAT_WINDOWS do
		...
		if i == 1 then
			...
			chat:SetWidth(ChatLeft:GetWidth() - 6)
			chat:SetHeight(ChatLeft:GetHeight() - 6)
 
			--FCF_SavePositionAndDimensions(chat)
		elseif i == 4 and name == "Loot" then
			chat:ClearAllPoints()
			chat:SetPoint("BOTTOM", ChatRight, "BOTTOM", 0, TukuiDB.Scale(4))
			chat:Hide()
			--FCF_SavePositionAndDimensions(chat)
		end
	end
...
function TukuiDB.ChatCopyButtons()
	for i = 1, NUM_CHAT_WINDOWS do
		...
		button:SetPoint("BOTTOMRIGHT", -1, -2)
