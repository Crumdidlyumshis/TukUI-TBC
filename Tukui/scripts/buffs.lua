...
local function UpdateDebuffAnchors(buttonName, index, filter)
...
local function UpdateBuffAnchors(buttonName, index, filter)
	if not (filter == "HELPFUL") then
		UpdateDebuffAnchors(buttonName, index, filter)
		return
	end
	buttonName = "BuffButton"
	local buff, previousBuff, aboveBuff;
	local numBuffs = 0;
	for index=1, BUFF_ACTUAL_DISPLAY+1 do
		local buff = _G[buttonName..index]
