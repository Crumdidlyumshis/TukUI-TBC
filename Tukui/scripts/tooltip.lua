...
hooksecurefunc("GameTooltip_SetDefaultAnchor", function(self, parent)
	if db.cursor == true then
		...
		self:SetPoint("BOTTOMRIGHT", ChatLeft, "TOPRIGHT", 0, TukuiDB.Scale(3))
	end
	self.default = 1
end)
	...
	elseif self:GetAnchorType() == "ANCHOR_NONE" then
		if InCombatLockdown() and db.hidecombat == true then
			...
			if TukuiCF["bags"].enable == true and StuffingFrameBags:IsShown() then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMLEFT", StuffingFrameBags, "TOPLEFT", 0, TukuiDB.Scale(3))
			else
				self:ClearAllPoints()
				self:SetPoint("BOTTOMLEFT", ChatRight, "TOPLEFT", 0, TukuiDB.Scale(3))
			end
		end
	end
end)
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
	...
	local GetQuestDifficultyColor = GetDifficultyColor
